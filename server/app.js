...
app.get('/', (req, res) => { res.send('Hello from Express 5000!')})
...
var firebaseConfig = {
  ...
  app.put('/tables', function(req, res) {
    firebase.firestore().collection('tables').doc('table'+req.body.tableID).update(req.body).then(res.send())
  })
  
  app.put('/add-menu', function(req, res) {
    firebase.firestore().collection('menu').doc(req.body.menuData.id).set(req.body.menuData).then(res.send())
    res.send();
  })
...
app.get('/menu', function(req, res) {
    var menuData = [];
    firebase.firestore().collection('menu').get().then((docs)=>{
        docs.forEach(doc => {
          var data = {};
          data = doc.data();
          var profit = doc.data().totalSales*doc.data().salePrice-doc.data().totalSales*doc.data().costPrice;
          data.profit = profit;
          menuData.push(data);
        })
        res.send(menuData)
...
app.put('/cleanTable/:tableID', function(req, res) {
  ...
  firebase.firestore().collection('tables').doc('table'+req.params.tableID).update(data)
  res.send();
})
...
app.delete('/waiting-list/:id', function(req, res) {
  firebase.firestore().collection('waiting-list').doc(req.params.id+"").delete().then(res.send())
})
